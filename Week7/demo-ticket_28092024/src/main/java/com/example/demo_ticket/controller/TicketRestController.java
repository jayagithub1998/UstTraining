...
public class TicketRestController {
	...
	ResponseEntity<List<Ticket>> getFromToTicket(@PathVariable("fromplace") String fromplace,@PathVariable("toplace") String toplace){
		List<Ticket> lticket=tService.getTicketFromToPlace(fromplace, toplace);
		System.out.println("Controller"+lticket);
		if(lticket.isEmpty()) {
			throw new InvalidPlaceNameException("No tickets found for the given places.");
		}
