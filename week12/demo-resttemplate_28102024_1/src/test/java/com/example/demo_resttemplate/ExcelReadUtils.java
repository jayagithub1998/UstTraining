...
public class ExcelReadUtils {
	...
	public static Stream<Arguments> readTicketJson() {
		...
		try {
			...
			for (int i = current_row_num; i < (current_row_num+num_of_testcases); i++) {
				...
				Cell celljson = rowi.getCell(1);
				String ticketjson = celljson.getStringCellValue();

				Cell celluser = rowi.getCell(2);
				String username = celluser.getStringCellValue();

				// add each Test data bundle to List
				args.add(Arguments.of(testcaseid,ticketjson, username));
			}
				current_row_num = current_row_num + num_of_testcases;
	...
	public static Stream<Arguments> updateTicket() {
		Row row = sheet.getRow(current_row_num++);
		Cell cell1 = row.getCell(0);
		int num_of_testcases = (int) cell1.getNumericCellValue();
		System.out.println("num_of_testcases" + num_of_testcases);
		...
		try {
			...
			for (int i = current_row_num; i < (current_row_num+num_of_testcases); i++) {
				...
				Cell cellticket = rowi.getCell(2);
				String ticket = cellticket.getStringCellValue();
				System.out.println("cellticket "+cellticket);
				
				// read second col - fromplace
				Cell cellfp = rowi.getCell(3);
				String fromplace = cellfp.getStringCellValue();
				System.out.println("fromplace "+fromplace);
				
				// read third col - toplace
				Cell celltp = rowi.getCell(4);
				String toplace = celltp.getStringCellValue();
				System.out.println("toplace "+toplace);
				
				Cell cellprice = rowi.getCell(5);
				float price = (float) cellprice.getNumericCellValue();
				System.out.println("price "+price);
				
				// add each Test data bundle to List
				args.add(Arguments.of(testcaseid,ticketid,ticket, fromplace, toplace, price));
			}
			current_row_num = current_row_num + num_of_testcases;
		} catch (Exception e) {
			System.out.println("Exceptionnnn:" + e.getMessage());
	...
	public static Stream<Arguments> deleteTicket() {
		...
		try {
			...
			for (int i = current_row_num; i < (current_row_num+num_of_testcases); i++) {
				...
				Cell cellticketid = rowi.getCell(1);
				int ticketid = (int) cellticketid.getNumericCellValue();

			// add each Test data bundle to List
				args.add(Arguments.of(testcaseid,ticketid));
			}
				current_row_num = current_row_num + num_of_testcases;
